...
убрана связь с проектом у пользователей, добавлено автоматическое создание суперпользователя root root(лог. пар.), отредактированы модели юзера(из ролей оставлены только Тестировщик и Главный тестировщик)
добавлено окно добавления кейсов, но пока только на тестовые наборы - посмотрите, скажите(единственное, что форма не закрывается после внесения изменений, но это будет дописано утром)

теперь runtime у тестовых наборов при изменениях ставится на сегодняшнюю дату

осталось автоматическое проставление автора и шаги, насколько помню
